...
class ORAMServer:
    ...
    def oramServerInitialization(self):
        ...
        self.tcpSoc.sendMessage(str(len(self.ES))+" "+str(len(self.EB))+" "+str(self.full[0]))
    ...
    def oramServerAccess(self):
        ...
        """
        if len(self.EB)>1:
            for i in range(len(self.EB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.packToStr(self.EB[i]))
        if len(self.ES)>1:
            for i in range(1,len(self.ES)):
                self.tcpSoc.sendMessage(cutils.packToStr(self.ES[i]))
        ...
        if len(self.EB)>1:
            ...
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TB),0,k_0)
        ...
        if len(self.ES)>1:
            ...
            modV = cutils.pirstrToVEll(strModV)
            prgK, convertK, k_0 = cutils.strToDpfKeys(strDpfK)
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TS),0,k_0)
        ...
        if self.full[0]==1:
            ...
            Bi0 = cutils.dpfEvalAll(prgK0,convertK0,len(self.EC[0]),0,k_00)
            val0 = cutils.readWithPos(2,Bi0,self.EC[0],self.posDict[0][0])
            
            prgK1, convertK1, k_01 = cutils.strToDpfKeys(strDpfK1)
            Bi1 = cutils.dpfEvalAll(prgK1,convertK1,len(self.EC[1]),0,k_01)
        ...
        for lev in range(self.ell+1,self.L+1):
            ...
            Bi0 = cutils.dpfEvalAll(prgK0,convertK0,len(self.ET[lev-self.ell][0]),0,k_00)
            
            val0 = cutils.readWithPos(2,Bi0,self.ET[lev-self.ell][0],self.posDict[lev-self.ell][0])
            
            
            prgK1, convertK1, k_01 = cutils.strToDpfKeys(strDpfK1)
            Bi1 = cutils.dpfEvalAll(prgK1,convertK1,len(self.ET[lev-self.ell][1]),0,k_01)
        ...
        if self.full[0]==1:
            ...
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TC[0]),0,k_0)
            ...
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TC[1]),0,k_0)
        ...
        for lev in range(self.ell+1,self.L+1):
            ...
            modV0 = cutils.pirstrToVEll(strModV0)
            prgK, convertK, k_0 = cutils.strToDpfKeys(strDpfK0)
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TT[lev-self.ell][0]),0,k_0)
            cutils.pirWriteWithEvalAndPos(1,Bi,modV0,self.TT[lev-self.ell][0],self.posDict[lev-self.ell][0])

            modV1 = cutils.pirstrToVEll(strModV1)
            prgK, convertK, k_0 = cutils.strToDpfKeys(strDpfK1)
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TT[lev-self.ell][1]),0,k_0)
    ...
    def oramServerRebuildFL(self):
        ...
        if len(tmpEB)>1:
            for i in range(len(tmpEB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.packToStr(tmpEB[i])) # send (k,v)
                self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTB[i])) # send tag
        if len(tmpES)>1:
            for i in range(1,len(tmpES)):
                self.tcpSoc.sendMessage(cutils.packToStr(tmpES[i])) # Only Server1
                self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTS[i]))              
        for i in dict.keys(tmpDict[0]):
            self.tcpSoc.sendMessage(cutils.packToStr(tmpEC[0][i])) # Only Server1
            self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTC[0][i]))
        for i in dict.keys(tmpDict[1]):
            self.tcpSoc.sendMessage(cutils.packToStr(tmpEC[1][i])) # Only Server1
        ...
        self.full[0] = 1
        self.ellAccessTimes = 0
        self.tcpSoc.sendMessage("Done"+" "+str(len(self.ES)))
    ...
    def oramServerRebuild(self, lev):
        ...
        if len(tmpEB)>1:
            for i in range(len(tmpEB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.packToStr(tmpEB[i])) # Only Server1
        ...
        for j in range(self.ell+1,lev):
            for i in dict.keys(self.posDict[j-self.ell][0]):
                self.tcpSoc.sendMessage(cutils.packToStr(self.ET[j-self.ell][0][i])) # Only Server1
                self.tcpSoc.sendMessage(cutils.bytesToStr(self.TT[j-self.ell][0][i]))
            for i in dict.keys(self.posDict[j-self.ell][1]):
                self.tcpSoc.sendMessage(cutils.packToStr(self.ET[j-self.ell][1][i])) # Only Server1
        ...
        self.full[lev-self.ell] = 1
        self.ellAccessTimes = 0
        self.tcpSoc.sendMessage("Done"+" "+str(len(self.ES))+" "+str(self.full[0]))
    ...
    def oramServerRebuildL(self):
        ...
        self.full[self.L-self.ell]=1
        self.ellAccessTimes = 0
        self.tcpSoc.sendMessage(str(len(self.ES))+" "+str(self.full[0]))
