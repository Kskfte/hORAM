...
class ORAMClient:
    ...
    def oramClientAccess(self, op, a, writeV):
        ...
        self.clientAcessStorage = max(len(posList)+2,self.clientAcessStorage)
        self.ctr += 1
        ...
        while True:
            ...
            if tes[0] == "Done":
                for mess in tempD:
                    ...
                    self.sendBlockNum += 2
                self.lenES=int(self.receiveMAndSplit(self.tcpSoc0)[0])
                self.lenEBAndTB=1
                self.full[0]=1
                break
            ...
            tempD.append(recK+" "+recV+" "+str(posEllLevel0)+" "+str(posEllLevel1)+"\n")
            if len(tempD)==self.L:
                for mess in tempD:
                    """
                    send to server
        ...
        while True:
            ...
            if tes[0] == "Done":
                for mess in tempD:
                    ...
                    self.tcpSoc1.sendMessage(mess)
                    self.sendBlockNum += 2
                lenFull = self.receiveMAndSplit(self.tcpSoc0)
            ...
            tempD.append(recK+" "+recV+" "+str(posLev0)+" "+str(posLev1)+" "+str(posEll0)+" "+str(posEll1)+"\n")
            if len(tempD)==self.L:
                for mess in tempD:
                    """
                    send to server
    ...
    def oramClientRebuildL(self):
        ...
        for i in range(self.N):
            (tmpK,tmpV) = self.oramClientOnlyRead(str(i+1))
...
if __name__=="__main__":
    ...
    for i in range(access_times):
        ...
        clientAccessStoList.append(coram.clientAcessStorage)
        #print(coram.clientAcessStorage)
    ...
    pic = open(r'C:\Users\zxl\Desktop\LORAM\GitRespositery\hORAM\Result\OursLogBlockNum_{}.pkl'.format(NN), 'wb') #open(r'.\Ours\Result\BlockNum_{}.pkl'.format(NN), 'wb')
