...
class ORAMClient:
    ...
    def oramClientAccess(self, op, a, writeV):
        ...
        if self.ctr%(2**self.L)==0:
            #print("rebuildLevel:{}".format(self.L))
            ...
            #print("rebuildLevel:{}".format(self.ell))
    ...
    def oramClientRebuildFL(self):
        ...
        while True:
            tes = self.receiveM(self.tcpSoc0)#self.tcpSoc0.receiveMessage()

            if tes[:4] == "Done":
                self.lenES=int(tes[4:])
                self.lenEBAndTB=1
                self.full[0]=1
                break
            
            (recK,recV) = int(tes[:self.AddrSize]),str(tes[self.AddrSize:])
            tmpTag0 = cutils.strToBytes(self.receiveM(self.tcpSoc0))
            tmpTag1 = cutils.strToBytes(self.receiveM(self.tcpSoc1))
            ...
            dataMessage0 = cutils.padToSize(str(recK),self.AddrSize)+str(recV)+cutils.padToSize(str(posEllLevel0),self.PosInTabSize)+cutils.padToSize(str(posEllLevel1),self.PosInTabSize)
            dataMessage1 = cutils.padToSize(str(recK),self.AddrSize)+str(recV)+cutils.padToSize(str(posEllLevel0),self.PosInTabSize)+cutils.padToSize(str(posEllLevel1),self.PosInTabSize)
    ...
    def oramClientRebuild(self, lev):
        ...
        while True:
            ...
            if tes[:4] == "Done":
                self.full[0]=int(tes[4])
                self.lenES=int(tes[5:])
                self.lenEBAndTB=1
                break
            ...
            dataMessage0 = cutils.padToSize(str(recK),self.AddrSize)+str(recV)+cutils.padToSize(str(posLev0),self.PosInTabSize)+cutils.padToSize(str(posLev1),self.PosInTabSize)+cutils.padToSize(str(posEll0),self.PosInTabSize)+cutils.padToSize(str(posEll1),self.PosInTabSize)
            dataMessage1 = cutils.padToSize(str(recK),self.AddrSize)+str(recV)+cutils.padToSize(str(posLev0),self.PosInTabSize)+cutils.padToSize(str(posLev1),self.PosInTabSize)+cutils.padToSize(str(posEll0),self.PosInTabSize)+cutils.padToSize(str(posEll1),self.PosInTabSize)
...
if __name__=="__main__":
    ...
    access_times = 3#32#2*NN-1#1#len(A)//2 513#
    ...
    pic = open('/home/zxl/local/hORAM/Ours/Result/Ours_BlockNum{}_Blockize{}.pkl'.format(NN,BlockSize), 'wb') #open(r'.\Ours\Result\BlockNum_{}.pkl'.format(NN), 'wb')
