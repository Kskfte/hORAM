...
def packMToStr(tagKV):
    return bytesToStr(tagKV[0])+" "+str(tagKV[1])+" "+str(tagKV[2])+" "

def unpackStrToM(message):
    data = message.split()
    return strToBytes(data[0]),str(data[1]),str(data[2])
...
def standardHash(tagKV, pos, table, stash):
    if (bytes(16),"-1","-1") in table[pos]:
        table[pos][table[pos].index((bytes(16),"-1","-1"))] = tagKV
    else:
        stash.append(tagKV)

def cuckooHash(tagKV, pos0, pos1, table, stash, posDict, thresholdKicked):
    ins_preservedV = tagKV
    ins_table_num = 0
    kickedPos = -1
    writePos = -1
    Loc = [pos0, pos1]
    if table[0][Loc[0]]==(bytes(16),"-1","-1"):
        table[0][Loc[0]]=ins_preservedV
        posDict[0][Loc[0]]=Loc[1]
        return
    elif table[1][Loc[1]]==(bytes(16),"-1","-1"):
    ...
    while count<thresholdKicked-1:
        if table[ins_table_num][writePos]==(bytes(16),"-1","-1"):
