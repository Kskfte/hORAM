        ...
        for lev in range(3,self.maxLevel+1,2):
            ...
            if lev<self.ellCuckoo:
                ...
                for i in range(self.eachBucketCapacity):
                    self.tcpSoc.sendMessage(self.packMToStr(self.server1Table[(lev-1)//2][pos][i]))
                    self.server1Table[(lev-1)//2][pos][i] = self.unpackStrToM(self.tcpSoc.receiveMessage())
            else:
                pos0 = int(self.tcpSoc.receiveMessage())
                pos1 = int(self.tcpSoc.receiveMessage())
                self.tcpSoc.sendMessage(self.packMToStr(self.server1Table[(lev-1)//2][0][pos0]))
                self.tcpSoc.sendMessage(self.packMToStr(self.server1Table[(lev-1)//2][1][pos1]))
                self.server1Table[(lev-1)//2][0][pos0] = self.unpackStrToM(self.tcpSoc.receiveMessage())
                self.server1Table[(lev-1)//2][1][pos1] = self.unpackStrToM(self.tcpSoc.receiveMessage())
        
        for i in range(len(self.stash1)):
            self.tcpSoc.sendMessage(self.packMToStr(self.stash1[i]))
            self.stash1[i] = self.unpackStrToM(self.tcpSoc.receiveMessage())
        self.tcpSoc.sendMessage("Done")

        if self.availS:
            self.stash1[self.stash1.index(self.emptyForm)] = self.unpackStrToM(self.tcpSoc.receiveMessage())
