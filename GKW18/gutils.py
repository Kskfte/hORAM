...
def unpackStrToKV(Str):
    data = Str.split( )
    return str(data[0]),str(data[1])

def packKVToStr(kVPos):
    return str(kVPos[0])+" "+str(kVPos[1])

def unpackStrToKVPos(Str):
    data = Str.split( )
    return str(data[0]),str(data[1]),int(data[2])

def packKVPosToStr(kVPos):
    return str(kVPos[0])+" "+str(kVPos[1])+" "+str(kVPos[2])
