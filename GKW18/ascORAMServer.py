...
class aORAMServer:
    ...
    def aORAMServerInitialization(self):
        ...
        for _ in range(self.N):
            ...
            for i in range(self.treeDepth,0,-1):
                nowLevPos = gutils.leafPosTolevPos(leafPos,i,self.treeDepth)#leafPos//(self.treeDepth-i)
                ...
                if tempWriteFlag:
                    break
            if not tempWriteFlag:
                temp_stash.append((k,v))
        
        for (k,v) in temp_stash:
            self.tcpSoc.sendMessage(gutils.packKVToStr((k,v)))
        self.tcpSoc.sendMessage("Done")

    def aORAMServerAccess(self):
        pk,ck,k0 = gutils.strToDpfKeys(self.tcpSoc.receiveMessage())
        Bi = gutils.dpfEvalAll(pk,ck,self.leafNum,0,k0)
    ...
    def aORAMServerEvict(self):
        ...
        for i in range(1,self.treeDepth+1):
            ...
            for j in range(self.Z):
                self.tcpSoc.sendMessage(gutils.packKVToStr(self.Tree[i][pos][j]))
